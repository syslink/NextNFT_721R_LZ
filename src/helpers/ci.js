...
const CI = (function () {
		...
		mosaic: function (opts, callback) {
			...
			for (var x = 0, p1 = 0, ix = 0; x < height; x += size, p1 += s1, ix++) {
				for (var y = 0, p2 = p1, iy = 0; y < width; y += size, p2 += s2, iy++) {
					...
					// 将颜色值设置回ImageData
					if (!opts.notWrite) {
						for (var ii = 0, pp3 = p2; ii < size_h; ii++ , pp3 += ww) {
							for (var jj = 0, pp4 = pp3; jj < size_w; jj++ , pp4 += 4) {
								pixes[p4] = temp[0];
								pixes[p4 + 1] = temp[1];
								pixes[p4 + 2] = temp[2];
							}
					...
					if (indexTable[y][x] && indexTable[y][x][0] == 0) {
						...
						if (word.length) {
							...
							if (word) {
								...
								// 清空已被占用的点
								var len_ii = isHor ? word[3] : word[5],
									len_jj = isHor ? word[2] : word[4];
								for (var ii = 0; ii < len_ii; ii++) {
									for (var jj = 0; jj < len_jj; jj++) {
										indexTable[y + ii] && (indexTable[y + ii][x + jj] = null);
									}
								}
	...
	return {
		...
		woodcut: function (canvas, opts) {
			...
			var pG = opts.pG || 0;
			var pB = opts.pB || 0;
			var pA = (opts.pA !== undefined) ? opts.pA : 255;
			var bR = (opts.bR !== undefined) ? opts.bR : 255;
			var bG = (opts.bG !== undefined) ? opts.bG : 255;
			var bB = (opts.bB !== undefined) ? opts.bB : 255;
			var bA = (opts.bA !== undefined) ? opts.bA : 255;
				...
				if (isImg(word)) {
					...
					fontFamily.forEach(function (f) {
						...
						for (var ii = 0; ii < len; ii++) {
							var w = ctx.measureText(word.charAt(ii)).width;
							width += w;
							if (singleWidth < w) {
								singleWidth = w;
							}
						...
						// 计算出在不同字体倍数下文字绘制大小
						for (var j = fontZoom; j > 0; j--) {
							re.push([
								word,	// 文本
								fontList[j],	// 字体
								width * j,	// 宽度单位（水平）
								j,	// 倍数|高度单位（水平）
								singleWidth * j,	// 宽度单位（垂直）
								len * j,	// 高度单位（垂直）
								dev[f] ? parseInt(dev[f] * fontSize * j) : 0	// 字体渲染偏移
							]);
						}
					});
				}
		...
		createCharImg: function (canvas, opts, callback) {
			...
			var wordList = opts.wordList,
				...
				square = opts.square !== undefined ? opts.square : setting.square,
				useOnes = opts.useOnes !== undefined ? opts.useOnes : setting.useOnes,
			...
			series(setting.async, [
				...
				function (data, next) {
					...
					if (img && img.tagName === "IMG") {
						var c = ci.loadImgToCanvas(img, {canvas: canvas, width: img.width, height: img.height });
						opts.img = c.canvas;
					}
					next();
				},
